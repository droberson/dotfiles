...
DOTFILES="bashrc profile fvwmrc mailcap mime.types zshrc kshrc emacs
...
for file in $PWD/zsh.d/*.zsh; do
    echo -n "  [*] $file "
    if [ -h "$HOME/.zsh.d/$(basename "$file")" ]; then
	echo "is a symbolic link. Skipping."
    else
	echo "not installed yet. applying symlink."
	ln -s "$file" "$HOME/.zsh.d/$(basename "$file")"
    fi
done
